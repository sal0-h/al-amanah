...
import { Plus, Edit2, Trash2, ChevronDown, ChevronRight, Calendar, MapPin, FileText, Clock } from 'lucide-react';
...
export default function AdminPanel() {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'roster' | 'users' | 'teams' | 'templates'>('roster');
    ...
    <div className="min-h-screen bg-stone-50">
      <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <a href="/dashboard" className="text-sm text-primary-500 hover:text-primary-500 font-medium transition-colors">← Back to Dashboard</a>
        </div>
        <div className="max-w-6xl mx-auto px-4">
          <nav className="flex gap-4">
            <button
              onClick={() => setActiveTab('roster')}
              className={`py-2 px-1 border-b-2 text-sm font-medium transition-colors ${activeTab === 'roster' ? 'border-primary-500 text-primary-500' : 'border-transparent text-gray-500 hover:text-primary-400'}`}
            >
              Roster Management
            </button>
            <button
              onClick={() => setActiveTab('users')}
              className={`py-2 px-1 border-b-2 text-sm font-medium transition-colors ${activeTab === 'users' ? 'border-primary-500 text-primary-500' : 'border-transparent text-gray-500 hover:text-primary-400'}`}
            >
              Users
            </button>
            <button
              onClick={() => setActiveTab('teams')}
              className={`py-2 px-1 border-b-2 text-sm font-medium transition-colors ${activeTab === 'teams' ? 'border-primary-500 text-primary-500' : 'border-transparent text-gray-500 hover:text-primary-400'}`}
            >
              Teams
            </button>
            <button
              onClick={() => setActiveTab('templates')}
              className={`py-2 px-1 border-b-2 text-sm font-medium transition-colors ${activeTab === 'templates' ? 'border-primary-500 text-primary-500' : 'border-transparent text-gray-500 hover:text-primary-400'}`}
            >
              Templates
            </button>
          </nav>
        ...
        <div className="border-t">
          ...
          <div className="flex border-b bg-gray-50">
            <button 
              ...
              className={`px-4 py-2 text-sm font-medium ${activeSection === 'weeks' ? 'border-b-2 border-primary-500 text-primary-500' : 'text-gray-500'}`}
            >
              📅 Weeks & Events
            </button>
            <button 
              onClick={() => setActiveSection('roster')} 
              className={`px-4 py-2 text-sm font-medium ${activeSection === 'roster' ? 'border-b-2 border-primary-500 text-primary-500' : 'text-gray-500'}`}
            >
              👥 Roster
            </button>
          </div>
      ...
      {expanded && (
        <div className="border-t p-3 bg-gray-50">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowWeekTemplateForm(true)} className="text-xs text-purple-600 hover:underline">📅 Week Template</button>
              <button onClick={() => setShowTemplateForm(true)} className="text-xs text-green-600 hover:underline">📋 Event Template</button>
  ...
  return (
    <div className="bg-white rounded-xl border p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowBatchForm(true)} className="flex items-center gap-1 px-3 py-1.5 text-sm border border-primary-500 text-primary-500 rounded-lg hover:bg-primary-50">
            📋 Batch Import
          </button>
            ...
            <div key={template.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {template.is_custom ? (
                    <span className="text-xs px-2 py-0.5 bg-accent-100 text-accent-700 rounded-full">Custom</span>
                  ) : (
                    <span className="text-xs px-2 py-0.5 bg-gray-200 text-gray-600 rounded-full">Default</span>
                  )}
                </div>
      ...
      <div className="bg-white rounded-xl border p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-semibold">Week Templates</h3>
          </div>
          <button
    ...
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto">
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          {tasks.length === 0 && (
            <p className="text-sm text-gray-500 text-center py-2">No tasks added yet</p>
          )}
        </div>
      </div>

      <div className="flex gap-3 pt-2">
        <button type="button" onClick={onCancel} className="flex-1 px-4 py-2 border rounded-lg hover:bg-gray-50">
          Cancel
        </button>
